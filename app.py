...
# Configure logging
logging.basicConfig(level=logging.DEBUG, filename='/var/www/packaging/whpackaging/debug.log', filemode='w', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def draw_layer(packages, layer_number, pallet_width=100, pallet_depth=100):
    try:
        ...
        logging.error(f"Error drawing layer: {e}")
        return None, []
...
def plan_pallet(height_limit, pallet_width=100, pallet_depth=100):
    ...
    try:
        ...
        logging.error(f"Error planning pallet: {e}")
        return []
